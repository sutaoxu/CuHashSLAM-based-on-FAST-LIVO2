...
public:
  ...
  vector<pointWithVar> _pv_list_cuda;
  ...
  StatesGroupCuda h_state_propagate;
  StatesGroup _state_cuda;
  ...
  VoxelMapManagerCudaPtr voxelmap_manager_cuda;
...
// 写 CSV 行辅助函数
template <typename T>
void write_csv_row(std::ofstream& ofs, const T& data, const std::string& prefix = "") {
...
// 写 CSV 值辅助函数（vector/matrix）
template <typename Derived>
void write_eigen_values(std::ofstream& ofs, const Eigen::MatrixBase<Derived>& mat) {
...
// 写 CSV 值辅助函数（C数组）
void write_array_values(std::ofstream& ofs, const double* arr, int size) {
...
// 主导出函数
void export_to_csv(const StatesGroup& cpu, const StatesGroupCuda& gpu, const std::string& filename) {
    ...
    // 写 header 行
    ofs << "rot_end[9],pos_end[3],vel_end[3],bias_g[3],bias_a[3],gravity[3],inv_expo_time,cov[361]\n";

    // 写 StatesGroup (Eigen)
    write_eigen_values(ofs, Eigen::Map<const VectorXd>(cpu.rot_end.data(), 9));
    ofs << ",";
    write_eigen_values(ofs, cpu.pos_end); ofs << ",";
    write_eigen_values(ofs, cpu.vel_end); ofs << ",";
    write_eigen_values(ofs, cpu.bias_g); ofs << ",";
    write_eigen_values(ofs, cpu.bias_a); ofs << ",";
    write_eigen_values(ofs, cpu.gravity); ofs << ",";
    ofs << cpu.inv_expo_time << ",";
    write_eigen_values(ofs, Eigen::Map<const VectorXd>(cpu.cov.data(), 361));
    ofs << "\n";

    // 写 StatesGroupCuda
    write_array_values(ofs, gpu.rot_end, 9); ofs << ",";
    write_array_values(ofs, gpu.pos_end, 3); ofs << ",";
    write_array_values(ofs, gpu.vel_end, 3); ofs << ",";
    write_array_values(ofs, gpu.bias_g, 3); ofs << ",";
    write_array_values(ofs, gpu.bias_a, 3); ofs << ",";
    write_array_values(ofs, gpu.gravity, 3); ofs << ",";
    ofs << gpu.inv_expo_time << ",";
    write_array_values(ofs, gpu.cov, 361);
    ofs << "\n";
