...
void loadVoxelConfigCuda(ros::NodeHandle &nh, VoxelMapConfigCuda &voxel_config_cuda)
{
    nh.param<bool>("publish/pub_plane_en", voxel_config_cuda.is_pub_plane_map_, false);
    
    nh.param<int>("lio/max_layer", voxel_config_cuda.max_layer_, 1);
    nh.param<double>("lio/voxel_size", voxel_config_cuda.max_voxel_size_, 0.5);
    nh.param<double>("lio/min_eigen_value", voxel_config_cuda.planner_threshold_, 0.01);
    nh.param<double>("lio/sigma_num", voxel_config_cuda.sigma_num_, 3);
    nh.param<double>("lio/beam_err", voxel_config_cuda.beam_err_, 0.02);
    nh.param<double>("lio/dept_err", voxel_config_cuda.dept_err_, 0.05);
    nh.param<vector<int>>("lio/layer_init_num", voxel_config_cuda.layer_init_num_, vector<int>{5,5,5,5,5});
    nh.param<int>("lio/max_points_num", voxel_config_cuda.max_points_num_, 50);
    nh.param<int>("lio/max_iterations", voxel_config_cuda.max_iterations_, 5);
  
    nh.param<bool>("local_map/map_sliding_en", voxel_config_cuda.map_sliding_en, false);
    nh.param<int>("local_map/half_map_size", voxel_config_cuda.half_map_size, 100);
    nh.param<double>("local_map/sliding_thresh", voxel_config_cuda.sliding_thresh, 8);
}
      ...
      extR(M3D::Identity())
{
  ...
  VoxelMapConfigCuda voxel_config_cuda;
  loadVoxelConfigCuda(nh, voxel_config_cuda);
  ...
  voxelmap_manager_cuda.reset(new VoxelMapManagerCuda(voxel_config_cuda));
  ...
  // voxelmap_manager->VoxelMemMalloc();
  // voxelmap_manager->StreamCreate();

  voxelmap_manager_cuda->VoxelMapMalloc();
  voxelmap_manager_cuda->HandleCreate();
  voxelmap_manager_cuda->StreamCreate();
}

LIVMapper::~LIVMapper() 
{
  // voxelmap_manager->VoxelMemRelease();
  voxelmap_manager_cuda->VoxelMapRelease();
  voxelmap_manager_cuda->HandleDestroy();
  savePCD();
}
...
void LIVMapper::initializeComponents() 
{
  ...
  M3D extR_transpose = extR.transpose();
  voxelmap_manager->extT_ << VEC_FROM_ARRAY(extrinT);
  voxelmap_manager->extR_ << MAT_FROM_ARRAY(extrinR);

  for (int i=0; i<9; i++)  {voxelmap_manager_cuda->extR_[i] = extR_transpose(i);}
  for (int i=0; i<3; i++)  {voxelmap_manager_cuda->extT_[i] = extT(i);}
...
void LIVMapper::processImu() 
{
  ...
  M3D rot_end_transpose = _state.rot_end.transpose();
  Eigen::Matrix<double, 19, 19> cov_transpose = _state.cov.transpose();
  eigenToCuda(rot_end_transpose, voxelmap_manager_cuda->state_.rot_end);
  eigenToCuda(_state.pos_end, voxelmap_manager_cuda->state_.pos_end);
  eigenToCuda(_state.vel_end, voxelmap_manager_cuda->state_.vel_end);
  eigenToCuda(_state.bias_g, voxelmap_manager_cuda->state_.bias_g);
  eigenToCuda(_state.bias_a, voxelmap_manager_cuda->state_.bias_a);
  eigenToCuda(_state.gravity, voxelmap_manager_cuda->state_.gravity);
  eigenToCuda(cov_transpose, voxelmap_manager_cuda->state_.cov);
  voxelmap_manager_cuda->state_.inv_expo_time = _state.inv_expo_time;
...
{    
  ...
  voxelmap_manager_cuda->feats_down_size_ = feats_down_size;
  voxelmap_manager_cuda->feats_down_world_size_ = feats_down_world->points.size();
  for (int i=0; i<feats_down_size; i++)
  {
    voxelmap_manager_cuda->h_feats_down_body_[i].x = feats_down_body->points[i].x;
    voxelmap_manager_cuda->h_feats_down_body_[i].y = feats_down_body->points[i].y;
    voxelmap_manager_cuda->h_feats_down_body_[i].z = feats_down_body->points[i].z;
    voxelmap_manager_cuda->h_feats_down_body_[i].normal_x = feats_down_body->points[i].normal_x;
    voxelmap_manager_cuda->h_feats_down_body_[i].normal_y = feats_down_body->points[i].normal_y;
    voxelmap_manager_cuda->h_feats_down_body_[i].normal_z = feats_down_body->points[i].normal_z;
    voxelmap_manager_cuda->h_feats_down_body_[i].curvature = feats_down_body->points[i].curvature;
    voxelmap_manager_cuda->h_feats_down_body_[i].intensity = feats_down_body->points[i].intensity;

    voxelmap_manager_cuda->h_feats_down_world_[i].x = feats_down_world->points[i].x;
    voxelmap_manager_cuda->h_feats_down_world_[i].y = feats_down_world->points[i].y;
    voxelmap_manager_cuda->h_feats_down_world_[i].z = feats_down_world->points[i].z;
    voxelmap_manager_cuda->h_feats_down_world_[i].normal_x = feats_down_world->points[i].normal_x;
    voxelmap_manager_cuda->h_feats_down_world_[i].normal_y = feats_down_world->points[i].normal_y;
    voxelmap_manager_cuda->h_feats_down_world_[i].normal_z = feats_down_world->points[i].normal_z;
    voxelmap_manager_cuda->h_feats_down_world_[i].curvature = feats_down_world->points[i].curvature;
    voxelmap_manager_cuda->h_feats_down_world_[i].intensity = feats_down_world->points[i].intensity;
  }

  voxelmap_manager_cuda->VoxelMemCpy(feats_down_size);

  if (!lidar_map_inited) 
  {
    lidar_map_inited = true;
    voxelmap_manager->BuildVoxelMap();
    voxelmap_manager_cuda->BuildVoxelMapCuda();
  }

  double t1 = omp_get_wtime();

  M3D rot_end_transpose = state_propagat.rot_end.transpose();
  Eigen::Matrix<double, 19, 19> cov_transpose = state_propagat.cov.transpose();
  eigenToCuda(rot_end_transpose, h_state_propagate.rot_end);
  eigenToCuda(state_propagat.pos_end, h_state_propagate.pos_end);
  eigenToCuda(state_propagat.vel_end, h_state_propagate.vel_end);
  eigenToCuda(state_propagat.bias_g, h_state_propagate.bias_g);
  eigenToCuda(state_propagat.bias_a, h_state_propagate.bias_a);
  eigenToCuda(state_propagat.gravity, h_state_propagate.gravity);
  eigenToCuda(cov_transpose, h_state_propagate.cov);
  h_state_propagate.inv_expo_time = state_propagat.inv_expo_time;

  voxelmap_manager->StateEstimation(state_propagat);
  _state = voxelmap_manager->state_;
  _pv_list = voxelmap_manager->pv_list_;

  voxelmap_manager_cuda->StateEstimationCuda(h_state_propagate);

  doubleToEigen(voxelmap_manager_cuda->state_.rot_end, _state_cuda.rot_end);
  _state_cuda.rot_end.transposeInPlace();
  doubleToEigen(voxelmap_manager_cuda->state_.pos_end, _state_cuda.pos_end);
  doubleToEigen(voxelmap_manager_cuda->state_.vel_end, _state_cuda.vel_end);
  doubleToEigen(voxelmap_manager_cuda->state_.bias_g, _state_cuda.bias_g);
  doubleToEigen(voxelmap_manager_cuda->state_.bias_a, _state_cuda.bias_a);
  doubleToEigen(voxelmap_manager_cuda->state_.gravity, _state_cuda.gravity);
  _state_cuda.inv_expo_time = voxelmap_manager_cuda->state_.inv_expo_time;
  doubleToEigen(voxelmap_manager_cuda->state_.cov, _state_cuda.cov);
  _state_cuda.cov.transposeInPlace();
  ...
  voxelmap_manager_cuda->PVListUpdateCuda();
  voxelmap_manager_cuda->PvListCpy(feats_down_size);

  voxelmap_manager->UpdateVoxelMap(voxelmap_manager->pv_list_);

  voxelmap_manager_cuda->UpdateVoxelMapCuda();
  ...
  // }
  //  voxelmap_manager_cuda->mapSlidingCuda();
